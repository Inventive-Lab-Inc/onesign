...
export function AccountUsersPanel() {
  const { workspaces, accountOwnerId, canAdminAccount, refreshWorkspaces } = useWorkspace();
  ...
  }, [canAdminAccount, loadUsers]);

  const changeAccess = useCallback(
    async (user: AccountUser, workspaceId: string, role: WorkspaceRoleOrNone) => {
      // Custom needs individual permissions, which can't be set from a single cell.
      if (role === "custom") {
        setEditingUser(user);
        return;
      }
      try {
        await persistWorkspaceAccess({ accountOwnerId, user, workspaceId, role });
        toast.success("Access updated");
        await Promise.all([loadUsers(), refreshWorkspaces()]);
      } catch (err) {
        toast.error(err instanceof Error ? err.message : "Could not update access");
      }
    },
    [accountOwnerId, loadUsers, refreshWorkspaces],
  );

  if (!canAdminAccount) {
    return (
      <p className="text-sm text-muted-foreground">
        You do not have permission to manage users for this account.
      </p>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground">Invite teammates and set their workspace access.</p>
                      ...
                      <td className="px-4 py-3">
                        {user.is_owner ? (
                          ...
                          <button
                            type="button"
                            onClick={() => setExpandedKey(expanded ? null : rowKey)}
                            className="flex max-w-md text-left"
                          >
