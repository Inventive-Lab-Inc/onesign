...
function PreviewSlide({
  ...
  return (
    ...
    <img src={url} alt="" decoding="async" className="h-full w-full object-cover bg-black" />
  );
}
...
const PREVIEW_FRAME_MAX_HEIGHT = "min(45vh, 360px)";

function PreviewAspectFrame({
  aspectW,
  aspectH,
  children,
}: {
    ...
    <div className="flex w-full justify-center">
      <div
        ...
        style={{
          ...
          width: `min(100%, calc(${PREVIEW_FRAME_MAX_HEIGHT} * ${aspectRatioNumber}))`,
...
export function PlaylistPreviewButton({
  ...
  const isDevicePreview = frame.kind === "device";
      ...
      {open && !empty && item ? (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4">
          ...
          <div
            ...
            className="relative z-10 flex w-full max-w-xl flex-col overflow-hidden rounded-xl border border-border bg-card shadow-lg"
          >
            <div className="flex shrink-0 items-start justify-between gap-4 border-b border-border bg-muted/30 px-5 py-4">
              <div className="min-w-0 space-y-0.5">
                <h2 id={titleId} className="text-lg font-semibold text-foreground">
                  {isDevicePreview ? "Screen preview" : "Playlist preview"}
                </h2>
                <p className="truncate text-sm text-muted-foreground">
                  {playlistName ?? "Playlist"}
                  {isDevicePreview && previewFrame.displayLabel ? (
                    <span className="text-muted-foreground/80"> · {previewFrame.displayLabel}</span>
                  ) : null}
                </p>
              </div>
            ...
            <div className="px-5 py-4 space-y-3">
              <PreviewAspectFrame aspectW={aspectW} aspectH={aspectH}>
                <PreviewSlide
                  key={`${item.id}-${index}`}
                  item={item}
                  onImageDone={advance}
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
                ...
                <span
                  className="truncate text-xs text-muted-foreground max-w-[min(100%,240px)]"
                  title={playlistItemTitle(item)}
                >
