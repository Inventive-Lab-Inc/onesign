...
declare
  ...
  v_slides jsonb;
  v_content_hash text;
  v_show_trial_watermark boolean := false;
  v_paused_by_quota boolean := false;
  v_owner_disabled boolean := false;
...
begin
  ...
  v_show_trial_watermark := public.account_shows_trial_watermark(v_owner);
  ...
  if v_playback_disabled then
    if v_owner_disabled then
      v_block_reason := 'account_suspended';
    elsif v_paused_by_quota then
      v_block_reason := 'paused_by_quota';
    else
      v_block_reason := 'admin_disabled';
    end if;
    ...
    return jsonb_build_object(
      ...
      'playbackBlockReason', to_jsonb(v_block_reason),
      ...
      'transitionStyle', to_jsonb('none'::text),
      'shuffleEnabled', to_jsonb(false),
      'showTrialWatermark', to_jsonb(v_show_trial_watermark)
    );
  end if;
  ...
  select
    coalesce(
      md5(
        ...
        || '|trial_wm|'
        || coalesce(v_show_trial_watermark::text, 'false')
      ),
      ''
    )
  into v_content_hash;
