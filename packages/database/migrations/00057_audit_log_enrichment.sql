-- Richer audit log display: waitlist context, metadata fallbacks, status transitions.
...
declare
  ...
  v_row public.access_waitlist%rowtype;
begin
  ...
  perform public.log_admin_action(
    ...
    jsonb_build_object(
      ...
      'screen_count', v_row.screen_count,
...
begin
  ...
  with filtered as (
    select
      ...
      coalesce(
        target_u.email::text,
        nullif(trim(l.metadata->>'email'), ''),
        w.email
      ) as target_email,
      coalesce(
        nullif(public.profile_display_name(target_p.client_name, target_u.email::text), ''),
        nullif(trim(l.metadata->>'client_name'), ''),
        nullif(trim(w.company_name), ''),
        nullif(trim(l.metadata->>'company_name'), ''),
        nullif(split_part(coalesce(target_u.email::text, l.metadata->>'email', w.email, ''), '@', 1), '')
      ) as target_client_name,
      l.metadata,
      l.created_at
    ...
    left join public.access_waitlist w
      on l.target_user_id is null
      and l.metadata ? 'waitlist_id'
      and w.id = (l.metadata->>'waitlist_id')::uuid
