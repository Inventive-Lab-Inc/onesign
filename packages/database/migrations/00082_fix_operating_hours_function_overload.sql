-- Migration 00081 added a 4-arg device_is_in_operating_hours while the 3-arg version from
-- 00078 remained. tv_get_playback_slides calls with 3 args, which PostgreSQL cannot resolve
-- ("function is not unique") — TVs then fail playback RPC and show "No playlist assigned".

drop function if exists public.device_is_in_operating_hours(jsonb, text, timestamptz);

-- Ensure tv_get_playback_slides passes the invert flag explicitly (4-arg only).
