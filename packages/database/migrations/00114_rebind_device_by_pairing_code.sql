-- Reconnect a player showing a pairing code to an existing linked screen without
-- creating a duplicate device row. Copies the pending player's identity onto the
-- target screen and removes the orphan pending row.
...
revoke all on function public.rebind_device_by_pairing_code(uuid, text, uuid) from public;
grant execute on function public.rebind_device_by_pairing_code(uuid, text, uuid) to authenticated;
