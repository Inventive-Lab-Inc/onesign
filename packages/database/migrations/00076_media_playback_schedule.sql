-- Enforce media/website schedule windows during TV playback (AbleSign-style).

create or replace function public.playback_schedule_is_active(
...
as $$
  select (p_display_from is null or p_at >= p_display_from)
     and (p_display_until is null or p_at <= p_display_until);
$$;

comment on function public.playback_schedule_is_active(timestamptz, timestamptz, timestamptz) is
  'True when p_at is within [display_from, display_until]; null bounds mean always active.';
...
revoke all on function public.playback_schedule_is_active(timestamptz, timestamptz, timestamptz) from public;
grant execute on function public.playback_schedule_is_active(timestamptz, timestamptz, timestamptz) to anon, authenticated;
