-- Enforce schedule windows when building TV playlists (media + websites).

create or replace function public.playback_item_is_active(
...
as $$
  select (p_display_from is null or p_display_from <= now())
     and (p_display_until is null or p_display_until > now());
$$;

comment on function public.playback_item_is_active(timestamptz, timestamptz) is
  'True when the current time is inside an optional start/expiry window. Null bounds mean always active.';
      ...
      md5(
        ...
        || coalesce((
            ...
            where pi.playlist_id = v_playlist_id
              and public.playback_item_is_active(pi.display_from, pi.display_until)
              and (
                (
                  pi.media_id is not null
                  and m.storage_path is not null
                  and length(trim(m.storage_path)) > 0
                  and public.playback_item_is_active(m.display_from, m.display_until)
                )
                or (
                  pi.website_id is not null
                  and w.playback_url is not null
                  and length(trim(w.playback_url)) > 0
                  and public.playback_item_is_active(w.display_from, w.display_until)
                )
              )
        ), '')
      ),
...
revoke all on function public.playback_item_is_active(timestamptz, timestamptz) from public;
grant execute on function public.playback_item_is_active(timestamptz, timestamptz) to anon, authenticated;
