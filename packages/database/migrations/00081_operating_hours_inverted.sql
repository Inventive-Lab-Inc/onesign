-- Inverted operating hours: screen active outside configured windows (AbleSign option).

alter table public.devices
  add column if not exists operating_hours_inverted boolean not null default false;

comment on column public.devices.operating_hours_inverted is
  'When true, the screen is in use outside the configured weekly windows instead of during them.';
...
as $$
  select case
    when coalesce(p_inverted, false) then
      not public.weekly_schedule_is_active_now(p_operating_hours, p_timezone, p_at)
    else
      public.weekly_schedule_is_active_now(p_operating_hours, p_timezone, p_at)
  end;
$$;

-- Re-apply tv_get_playback_slides with inverted hours support (from 00079 + invert flag).
...
declare
  ...
  v_operating_hours_inverted boolean := false;
  ...
  select
    coalesce(
      md5(
        ...
        || '|inv|'
        || coalesce(v_operating_hours_inverted::text, 'false')
        || '|'
        || coalesce((
            select string_agg(
              pi.id::text
