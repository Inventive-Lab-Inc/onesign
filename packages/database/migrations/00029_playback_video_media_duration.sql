-- Revision hash and TV slides use media.duration_seconds for videos, playlist_items.duration_seconds for images.

create or replace function public.playback_slide_duration_label(
  p_file_type public.media_file_type,
  p_playlist_duration integer,
  p_media_duration integer
)
returns text
language sql
immutable
as $$
  select case
    when p_file_type = 'video'::public.media_file_type
      then coalesce(p_media_duration::text, 'n')
    else coalesce(p_playlist_duration::text, 'n')
  end;
$$;

create or replace function public.playback_slide_duration_seconds(
  p_file_type public.media_file_type,
  p_playlist_duration integer,
  p_media_duration integer
)
...
as $$
  select case
    when p_file_type = 'video'::public.media_file_type then p_media_duration
    else p_playlist_duration
  end;
$$;

-- Patch revision fingerprint (same body as production; only duration segment changes).
