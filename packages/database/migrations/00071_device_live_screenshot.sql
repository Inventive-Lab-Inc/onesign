-- On-demand live TV screenshot: one overwrite object per device (not counted in media quota).

alter table public.devices
  add column if not exists screenshot_requested_at timestamptz,
  add column if not exists live_screenshot_at timestamptz;

comment on column public.devices.screenshot_requested_at is
  'When the console last asked this screen to capture a live frame.';
comment on column public.devices.live_screenshot_at is
  'When the TV last uploaded a live screenshot to object storage.';

create or replace function public.request_device_live_screenshot(p_device_id uuid)
returns void
language plpgsql
volatile
security definer
...
begin
  ...
  if not (
    (auth.uid() = v_owner and public.is_account_active())
...
revoke all on function public.request_device_live_screenshot(uuid) from public;
grant execute on function public.request_device_live_screenshot(uuid) to authenticated;

-- Include pending screenshot requests in the lightweight revision poll.
